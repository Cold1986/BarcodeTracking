...
{
    public partial class Upload_KS_Condiments_Out : System.Web.UI.Page
    {
        ...
        private void ddlBind()
        {
            ...
            this.ddlType.DataSource = ddlBiz.GetList(" type=2 order by sort");
            ...
            if (fileUpload.HasFile)
            {
                try
                {
                    ...
                    if (postedFileExtension == ".csv" || postedFileExtension == ".txt")
                    {
                        ...
                        //读取用户上传文件
                        DataTable dt = uploadFileBiz.GetdataFromCVS(fileUpload, userName, type, "out");
                        ...
                        if (dt != null && dt.Rows.Count > 0)
                        {
                            ...
                            for (int i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                else
                                {
                                    ...
                                    if ((tmpResult == null) || tmpResult.Tables.Count == 0)
                                    {
                                        result = false;
                                        errMsg += (i + 1).ToString() + "、";
                                    }
                                    else if (tmpResult.Tables[0].Rows.Count == 0 || userNumbers != Convert.ToInt32(tmpResult.Tables[0].Rows[0]["QTY"]))
                                    {
                                        result = false;
                                        errMsg += (i + 1).ToString() + "、";
                                    }
                                    ...
                                    {
                                        dt.Rows[i]["PKID"] = tmpResult.Tables[0].Rows[0]["PKID"];
                                        dt.Rows[i]["OPERORDER"] = tmpResult.Tables[0].Rows[0]["OPERORDER"];
                                        dt.Rows[i]["CDEFINE31"] = tmpResult.Tables[0].Rows[0]["CDEFINE31"];
                                    }
                                }
                            ...
                            if (result)
                            {
                                ...
                                string fileName = DateTime.Now.ToString("yyyyMMddHHmmssffff") + "_out_" + userName + "_" + postedFileName;
                                fileUpload.SaveAs(filePath + fileName);
                                //插入数据库
                                DataSet ds = new DataSet();
                                dt.TableName = "KGM_HISTORYBODY";
                                ds.Tables.Add(dt);
                                uploadFileBiz.UpdateByDataSet(ds, dt.TableName);
